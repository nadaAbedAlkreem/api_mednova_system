...
class StoreProgramRequest extends FormRequest
{
    ...
    {
        return [
            // بيانات البرنامج (Program)
            'creator_id' => 'required|exists:admins,id,deleted_at,NULL',
            ...
            // بيانات الفيديوهات المرتبطة (ProgramVideos)
            'videos' => 'nullable|array',
            'videos.*.title_ar' => 'required|string|max:255',
            'videos.*.title_en' => 'nullable|string|max:255',
            'videos.*.description_ar' => 'nullable|string',
            'videos.*.description_en' => 'nullable|string',
            'videos.*.video_path' => 'required|file|mimes:mp4,mov,avi,wmv,mkv|max:51200',
            'videos.*.duration_minute' => 'required|numeric|min:1',
            'videos.*.order' => 'nullable|integer|min:1',
            'videos.*.is_free' => 'boolean',
            'videos.*.is_program_intro' => 'boolean',
        ];
    }
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $orders = array_column($videos, 'order');
            $orders = array_filter($orders, fn($value) => !is_null($value));

            if (count($orders) !== count(array_unique($orders))) {
                $validator->errors()->add(
                    'videos',
                    __('validation.custom.videos.duplicate_order')
                );
            }


            $introCount = collect($videos)
                ->filter(fn($video) => !empty($video['is_program_intro']))
                ->count();

            if ($introCount === 0) {
                $validator->errors()->add(
                    'videos',
                    __('validation.custom.videos.intro_required')
                );
            }
            ...
            if ($introCount > 1) {
                $validator->errors()->add(
                    'videos',
                    __('validation.custom.videos.multiple_intro')
                );
            }
